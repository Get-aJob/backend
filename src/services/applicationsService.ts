...
function normalizeApplication(
  row: unknown,
  options: NormalizeApplicationOptions = {},
): any {
  if (!row) {
    return row;
  }
  ...
  const histories =
    includeHistories && Array.isArray(applicationStatusHistories)
...
async function insertStatusHistory(
  ...
  const { error } = await supabase.from(STATUS_TABLE_NAME).insert({
...
export async function getApplicationById(id: string): Promise<any> {
  const { data, error } = await supabase
    .from(TABLE_NAME)
    .select(
      "*, job_postings(title, content, company_name, deadline), application_statuses(display_name), application_status_histories(to_status_id,changed_at)",
    )
    .eq("id", id)
  ...
  if (data?.application_status_histories?.length) {
    ...
    if (toStatusIds.length > 0) {
      ...
      const statusNameMap = new Map(
        (statuses ?? []).map((status: any) => [status.id, status.display_name]),
      );

      data.application_status_histories = (
        data.application_status_histories as any[]
      ).map((history: any) => ({
        ...history,
        to_status_name: history?.to_status_id
          ? (statusNameMap.get(history.to_status_id) ?? null)
          : null,
      }));
    }
  }
...
export async function createApplication(
  ...
  if (statusId) {
    try {
      await insertStatusHistory(
        data.id,
        statusId,
        (application.user_id ?? application.userId) as string,
      );
    } catch (err) {
  ...
  const created = await getApplicationById(data.id);

  // 지원 내용 알림 생성
  const statusLabel = created?.statusName ?? statusId ?? "지원";
  const jobTitle = created?.jobPostings?.title ?? "공고";

  try {
    await createNotificationLog({
      userId: (application.user_id ?? application.userId) as string,
      type: "application_created",
      title: "지원 완료",
      body: `${jobTitle}에 ${statusLabel} 상태로 지원했습니다.`,
      payload: {
    ...
    console.error("알림 생성 실패(지원 생성은 유지):", e);
  }
  return created;
}
...
export async function getApplicationsByUser(
  ...
  const { data, error, count } = await supabase
    .from(TABLE_NAME)
    .select(
      "*, job_postings(title, content, company_name,deadline), application_statuses(display_name)",
      { count: "exact" },
    )
    .eq("user_id", userId)
  ...
  const convertedData = normalizeApplications(data ?? [], {
